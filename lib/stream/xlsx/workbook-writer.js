...
const Capacitor = require('../../utils/fs-capacitor');
...
class WorkbookWriter {
  constructor(options) {
    ...
    this.dontUseFsCapacitor = options.dontUseFsCapacitor === true;
  ...
  _openStream(path) {
    ...
    if (this.dontUseFsCapacitor) {
      this.zip.append(stream, {name: path});
    } else {
      // use fs-capacitor to handle large writes across multiple sheets
      const wstream = new Capacitor.WriteStream();
      Stream.pipeline(stream, wstream, err => {
        if (err) {
          // eslint-disable-next-line no-console
          console.error(err);
        }
      });
      this.zip.append(wstream.createReadStream(), {name: path});
    }
    stream.on('finish', () => {
      stream.emit('zipped');
    });
